											 ...
											 fSgAndBgNumbers(5)
{
  ...
  for (int i = 0; i < fNchan; ++i) {
    ...
    fCsNumbers[i].fNameMc = "nada";//bspsiphiMcComb";
    fCsNumbers[i].fNameDa = "nada";//bspsiphiData";
    ...
    fCombNumbers[i].fNameDa = "nada";//bmmData";
...
void plotResultsnew::makeAll(string what) {
  ...
  if (what == "bdtopt") {
    ...
    cout<<"Now calculating numbers"<<endl;
    for (int i = 0; i < fNchan; ++i) {
...
void plotResultsnew::fillAndSaveHistograms(int start, int nevents) {
  ...
  if (1) {
    ...
    //setupTree(t, fSample);
    //loopOverTree(C, 1, nevents, start);
    loopOverChain(C, 1, 60000, start);
    //loopOverChain(C, 1, nevents, start);
    saveHistograms(fSample);
...
void plotResultsnew::getAccAndEffFromEffTree(string ds, anaNumbers &a, cuts &b, int proc) {
  ...
  //TChain *tA=new TChain("candAnaBu2JpsiK/Events");
  //tA->Add("/eos/home-c/ckar/Bmm5_code/nanoaodsample/bmm-mc-BuToJpsiKp_nano2018.root");
  // TChain *tA=new TChain("tree");
  // if(ds == "bupsikMcCombAcc"){
  //   cout<<"BujpsikMC acc "<<endl;
  //   tA->Add("root://cms-xrd-global.cern.ch//store/user/paus/nanoao/503/BuToJpsiK_SoftQCDnonD_TuneCP5_13TeV-pythia8-evtgen+RunIIAutumn18MiniAOD-102X_upgrade2018_realistic_v15-v2+MINIAODSIM/*.root");
  // }
  // else if(ds == "bdmmMcCombAcc"){
  //   cout<<"Bdmm MC acc "<<endl;                                                                                                                                                          
  //   tA->Add("root://cms-xrd-global.cern.ch//store/user/paus/nanoao/503/BdToMuMu_SoftQCDnonD_TuneCP5_13TeV-pythia8-evtgen+RunIIAutumn18MiniAOD-102X_upgrade2018_realistic_v15-v2+MINIAODSIM/12228F43-BACC-D145-BE17-16C3501A74FB.root");                                                                                                                                                                                              
  // }                                                                                                                                                                                                              
  // else if(ds == "bsmmMcCombAcc"){
  //   cout<<"Bsmm MC acc "<<endl;                                                                                                                                                           
  //   tA->Add("root://cms-xrd-global.cern.ch//store/user/paus/nanoao/503/BsToMuMu_SoftQCDnonD_TuneCP5_13TeV-pythia8-evtgen+RunIIAutumn18MiniAOD-102X_upgrade2018_realistic_v15-v2+MINIAODSIM/0FB3B2A3-78BC-DD4C-9282-89F140CC8E62.root");                                                                                                                                                                                              
  // }
  ...
  Float_t bm1pt[14], bm1eta[14], bm2pt[14], bm2eta[14];
  Float_t bm[14];
  Float_t bk1pt[14], bk1eta[14];
  UInt_t nloop;
  Float_t bkmm_gen_pdgId[14];
  ULong64_t event;
  Int_t   bkmm_pdgId[14];   //[nbkmm]                                                                                                        
  Int_t   bkmm_kaon_pdgId[14];   //[nbkmm]
    
  tA->SetBranchAddress("event",&event);
  if(BU2JPSIKP == fMode){
    tA->SetBranchAddress("bkmm_jpsimc_mu1pt",&bm1pt);
    tA->SetBranchAddress("bkmm_jpsimc_mu2pt",&bm2pt);
    tA->SetBranchAddress("bkmm_jpsimc_mu1eta",&bm1eta);
    tA->SetBranchAddress("bkmm_jpsimc_mu2eta",&bm2eta);
    ...
    tA->SetBranchAddress("mm_kin_mu1pt",&bm1pt);
    tA->SetBranchAddress("mm_kin_mu2pt",&bm2pt);
    tA->SetBranchAddress("mm_kin_mu1eta",&bm1eta);
    tA->SetBranchAddress("mm_kin_mu2eta",&bm2eta);
  ...
  for (int jentry = 0; jentry < nentries; jentry++) {
    ...
    ++npre_l;     
      // -- Normalization
    for(UInt_t mom_l= 0. ; mom_l < nloop ; mom_l++){
      int bchan = -2;
      ++nprelm;
      
      if(max(TMath::Abs(bm1eta[mom_l]),TMath::Abs(bm2eta[mom_l])) < 0.7)bchan=0;
      else if(max(TMath::Abs(bm1eta[mom_l]),TMath::Abs(bm2eta[mom_l])) > 0.7 && max(TMath::Abs(bm1eta[mom_l]), TMath::Abs(bm1eta[mom_l])) < 1.4)bchan=1;

      //if(TMath::Abs(bkmm_pdgId[mom_l]) ==  521)cout<<"event "<<event<<" nbkmm "<<nbkmm<<" m1pt "<<bm1pt[mom_l]<<" m2pt "<<bm2pt[mom_l]<<" m1eta "<<bm1eta[mom_l]<<" m2eta "<<bm2eta[mom_l]<<" id "<<bkmm_pdgId[mom_l]<<" channel "<<bchan<<endl;
      if(no){
	//++ngen;
	if(TMath::Abs(bkmm_pdgId[mom_l]) ==  521 && TMath::Abs(bkmm_kaon_pdgId[mom_l]) == 321){
	  ++ngen;
	  if (bm1pt[mom_l] > fAccPt && bm2pt[mom_l]> fAccPt && bk1pt[mom_l] > 0.5 
	      && TMath::Abs(bm1eta[mom_l]) < fAccEtaRec && TMath::Abs(bm2eta[mom_l]) < fAccEtaRec 
	      && TMath::Abs(bk1eta[mom_l]) < fAccEtaRec){
      ...
      if(sg){
	Int_t PDGID ;
	if(string::npos != a.fName.find("bsmm")) PDGID = 531;
	else PDGID = 511;
	
	//++ngen;
	if(TMath::Abs(bkmm_pdgId[mom_l]) ==  PDGID){
	  ++ngen;
	  if (bm1pt[mom_l] > fAccPt && bm2pt[mom_l]> fAccPt
	      && TMath::Abs(bm1eta[mom_l]) < fAccEtaRec && TMath::Abs(bm2eta[mom_l]) < fAccEtaRec
	      ){
	    ++nreco0_pr;
	    if (bchan == a.fChan) {
  ...
  cout<<"#of event "<<npre_l<<" inside bkmmloop"<<nprelm<<" gen : "<<ngen<<" tree loop "<<nreco0_pr<<"\t final "<<ncand<<endl;
  //a.fAccGenFileYield.val = ngen;
  //a.fAccGenYield.val     = ngen;
  a.fAccGenFileYield.val = npre_l;
  a.fAccGenYield.val     = npre_l;
...
void plotResultsnew::numbersFromHist(anaNumbers &aa, string syst) {
  ...
  cout<<"MuonID Eff: "<<aa.fEffMuidMC.val<<" = "<<aa.fMuidYield.val<<" / "<<aa.fAnaYield.val<<endl;
  aa.fTrigYield.val        = d;
...
void plotResultsnew::loopFunction1() {
  ...
  fChan = -2;
  UInt_t nloop(0);
  //std::cout<<"Entering loopFunction1 "<<std::endl;
  if (BU2JPSIKP == fMode)  {
    nloop = fb.nbkmm;
    //std::cout<<" Considering Bu2JpsiK mode "<<fMode<<std::endl;
  }
  else if ((BSMM == fMode) || (BDMM == fMode) || (RARE == fMode)){
    nloop = fb.nmm; 
    //std::cout<<" Considering Bs2mm/Bd2mm/Rare mode "<<fMode<<std::endl;
  }
  
  for(UInt_t mom_l=0. ; mom_l<nloop ; mom_l++){
    
    //if(fb.mm_kin_mu1pt[mom_l] > fb.mm_kin_mu2pt[mom_l]){
    if(max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) < 0.7) fChan=0;
    else if(max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) > 0.7 && (max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) < 1.4)) fChan=1;
    else if(max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) > 1.4 && (max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) < 2.0)) fChan=2;
    else if(max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) > 2.0 && (max(fabs(fb.mm_kin_mu1eta[mom_l]),fabs(fb.mm_kin_mu2eta[mom_l])) < 2.5)) fChan=3;
	    
	    //cout<<"channel in plotClassnew "<<fChan<<endl;                                                                                                                   
    if (fChan < 0 || fChan >= fNchan) {
      if (0) cout << "plotClassnew::candAnalysis: " << fb.run << " " << fb.evt
		  << " could not determine channel: " << fb.mm_kin_mu1eta[mom_l] << " " << fb.mm_kin_mu2eta[mom_l] << endl;
      //fBDT = -99.;                                                                                                                                          
  ...
  Int_t bindex = 0, mmindex1=0, mmindex2=0;
  if(BU2JPSIKP == fMode){
    bindex = fb.bkmm_mm_index[mom_l];
    mmindex1 = fb.mm_mu1_index[bindex];
    mmindex2 = fb.mm_mu2_index[bindex];
    if(fIsMC){
      if(TMath::Abs(fb.bkmm_pdgId[mom_l]) == 521 && TMath::Abs(fb.mm_gen_pdgId[bindex]) == 443) fGoodDcand = true;
    }
    else{
      fGoodDcand = true;
    }
  } else {
    mmindex1 = fb.mm_mu1_index[mom_l];
    mmindex2 = fb.mm_mu2_index[mom_l];
    if(fIsMC){
      if(BSMM == fMode){if(TMath::Abs(fb.mm_gen_pdgId[mom_l]) == 531) fGoodDcand = true;}
      if(BDMM == fMode){if(TMath::Abs(fb.mm_gen_pdgId[mom_l]) == 511) fGoodDcand = true;}
    }
    else{
      fGoodDcand = true;
    }
  ...
  fGoodTracksPt   = true;
  fGoodTracksEta  = true;
  fGoodQ          = (fb.Muon_Charge[mmindex1]*fb.Muon_Charge[mmindex2]<0);
  fIsCowboy = true;
  
  if(!(fIsMC)){if((fb.m[mom_l]<4.9 || fb.m[mom_l]>6.0))return;}
  if (fIsMC) {
    if (fb.mm_kin_mu1pt[mom_l] < fAccPt) fGoodAcceptance = false;
    if (fb.mm_kin_mu2pt[mom_l] < fAccPt) fGoodAcceptance = false;
    if (TMath::Abs(fb.mm_kin_mu1eta[mom_l]) > fAccEtaGen) fGoodAcceptance = false;
    if (TMath::Abs(fb.mm_kin_mu2eta[mom_l]) > fAccEtaGen) fGoodAcceptance = false;
  } else {
    
  }

  double m1 = TMath::Abs(fb.mm_kin_mu1eta[mom_l]);
  double m2 = TMath::Abs(fb.mm_kin_mu2eta[mom_l]);
  if (m2 > m1) m1 = m2;
  ...
  double w1(-1.), w2(-1.);
  if(BU2JPSIKP == fMode){
    fGoodIp         = (TMath::Abs(fb.pvip[mom_l]) < pCuts->pvip);
    fGoodIpS        = (TMath::Abs(fb.pvip[mom_l]/fb.pvipErr[mom_l]) < pCuts->pvips);
    
    fGoodLip        = (TMath::Abs(fb.pvlip[mom_l]) < pCuts->pvlip);
    fGoodLipS       = (TMath::Abs(fb.pvlip[mom_l]/fb.pvlipErr[mom_l]) < pCuts->pvlips);
    
    fGoodPt         = (fb.pt[mom_l] > pCuts->pt);
    fGoodEta        = ((fb.eta[mom_l] > -fAccEtaRec) && (fb.eta[mom_l] < fAccEtaRec));
    fGoodAlpha      = (fb.alpha[mom_l] < pCuts->alpha);
    fGoodChi2       = (fb.chi2dof[mom_l] < pCuts->chi2dof);
    fGoodFLS        = (fb.fls3d[mom_l] > pCuts->fls3d);
    if (TMath::IsNaN(fb.fls3d[mom_l])) fGoodFLS = false;
    fGoodCloseTrack   = (fb.closetrk[mom_l] < pCuts->closetrk);
    fGoodCloseTrackS1 = (fb.closetrks1[mom_l] < pCuts->closetrks1);
    fGoodCloseTrackS2 = (fb.closetrks2[mom_l] < pCuts->closetrks2);
    fGoodCloseTrackS3 = (fb.closetrks3[mom_l] < pCuts->closetrks3);
    fGoodIso          = (fb.iso[mom_l] > pCuts->iso);
    fGoodM1Iso        = (fb.m1iso[mom_l] > pCuts->m1iso);
    fGoodM2Iso        = (fb.m2iso[mom_l] > pCuts->m2iso);
    fGoodDocaTrk      = (fb.docatrk[mom_l] > pCuts->docatrk);
    
    fGoodBDT        = (fb.bdt[mom_l] > pCuts->bdtCut);
    
    //fGoodCmssw   =  (fb.m1pt[mom_l] > 4.) && (fb.m2pt[mom_l] > 4.)   && (fb.flsxy[mom_l] > 4.) && (fb.docatrk[mom_l] < 0.08) && (fb.chi2dof[mom_l] < 5.);
    //fGoodCandAna =  (fb.alpha[mom_l] < 0.2) && (fb.fls3d[mom_l] > 4.) && (TMath::Abs(fb.pvip[mom_l]/fb.pvipErr[mom_l]) < 4.) && (TMath::Abs(fb.pvip[mom_l]) < 0.02);
    fGoodCmssw   =  (fb.m1pt[mom_l] > 4.) && (fb.m2pt[mom_l] > 4.)   && (fb.flsxy[mom_l] > 4.) && (fb.bkmm_kaon_sdxy_bs[mom_l] > 5.) && (fb.chi2dof[mom_l] < 5.);
    fGoodCandAna =  (fb.fls3d[mom_l] > 4.) && (fb.bkmm_jpsimc_vtx_prob[mom_l] > 0.1);
    // global definition
    // -- no trigger matching for rare decays!                                                                                                                         
    fGoodHLT        = fb.hlt1 ;
  } else {
    fGoodIp         = (TMath::Abs(fb.mm_kin_pvip[mom_l]) < pCuts->pvip);
    fGoodIpS        = (TMath::Abs(fb.mm_kin_pvip[mom_l]/fb.mm_kin_pvipErr[mom_l]) < pCuts->pvips);
    
    fGoodLip        = (TMath::Abs(fb.mm_kin_pvlip[mom_l]) < pCuts->pvlip);
    fGoodLipS       = (TMath::Abs(fb.mm_kin_pvlip[mom_l]/fb.mm_kin_pvlipErr[mom_l]) < pCuts->pvlips);
    
    fGoodPt         = (fb.mm_kin_pt[mom_l] > pCuts->pt);
    fGoodEta        = ((fb.mm_kin_eta[mom_l] > -fAccEtaRec) && (fb.mm_kin_eta[mom_l] < fAccEtaRec));
    fGoodAlpha      = (fb.mm_kin_alpha[mom_l] < pCuts->alpha);
    fGoodChi2       = (fb.mm_kin_vtx_chi2dof[mom_l] < pCuts->chi2dof);
    fGoodFLS        = (fb.mm_kin_sl3d[mom_l] > pCuts->fls3d);
    if (TMath::IsNaN(fb.mm_kin_sl3d[mom_l])) fGoodFLS = false;
    fGoodCloseTrack   = (fb.mm_closetrk[mom_l] < pCuts->closetrk);
    fGoodCloseTrackS1 = (fb.mm_closetrk1[mom_l] < pCuts->closetrks1);
    fGoodCloseTrackS2 = (fb.mm_closetrk2[mom_l] < pCuts->closetrks2);
    fGoodCloseTrackS3 = (fb.mm_closetrk3[mom_l] < pCuts->closetrks3);
    fGoodIso          = (fb.mm_iso[mom_l] > pCuts->iso);
    fGoodM1Iso        = (fb.mm_m1iso[mom_l] > pCuts->m1iso);
    fGoodM2Iso        = (fb.mm_m2iso[mom_l] > pCuts->m2iso);
    fGoodDocaTrk      = (fb.mm_docatrk[mom_l] > pCuts->docatrk);
    
    fGoodBDT        = (fb.mm_bdt[mom_l] > pCuts->bdtCut);
    
    //fGoodCmssw   =  (fb.mm_kin_mu1pt[mom_l] > 4.) && (fb.mm_kin_mu2pt[mom_l] > 4.)   && (fb.mm_kin_slxy[mom_l] > 4.) && (fb.mm_docatrk[mom_l] < 0.08) && (fb.mm_kin_vtx_chi2dof[mom_l] < 5.);
    //fGoodCandAna =  (fb.mm_kin_alpha[mom_l] < 0.2) && (fb.mm_kin_sl3d[mom_l] > 4.) && (TMath::Abs(fb.mm_kin_pvip[mom_l]/fb.mm_kin_pvipErr[mom_l]) < 4.) && (TMath::Abs(fb.mm_kin_pvip[mom_l]) < 0.02);
    fGoodCmssw   =  (fb.mm_kin_mu1pt[mom_l] > 4.) && (fb.mm_kin_mu2pt[mom_l] > 4.)   && (fb.mm_kin_slxy[mom_l] > 4.) && (fb.mm_kin_vtx_chi2dof[mom_l] < 5.);
    fGoodCandAna =  (fb.mm_kin_sl3d[mom_l] > 4.);
  ...
  fGoodMuonsID = ( fGoodMuonsPt && fGoodMuonsEta && (fb.Muon_softId[mmindex1]==1) && (fb.Muon_softId[mmindex2]==1));

  //fGoodMuonsID = ( fb.Muon_isGlobal[mmindex1] && fb.Muon_isGlobal[mmindex2] && fGoodMuonsPt && fGoodMuonsEta && (Muon_softId[mmindex1]==1) && (Muon_softId[mmindex2]==1));// && (fb.Muon_mvaLowPt[mmindex1] > fCuts[0]->muonbdt) && (fb.Muon_mvaLowPt[mmindex2] > fCuts[0]->muonbdt));
  //fGoodMuonsID = (fb.Muon_isGlobal[mmindex1] && fb.Muon_isGlobal[mmindex2] &&fGoodMuonsPt && fGoodMuonsEta && (fb.Muon_mvaLowPt[mmindex1] > fCuts[0]->muonbdt) && (fb.Muon_mvaLowPt[mmindex1] > fCuts[0]->muonbdt));

  fGoodRealMuonsID = ((fb.Muon_mvaLowPt[mmindex1] > -2.5) && (fb.Muon_mvaLowPt[mmindex1] > -2.5) && fGoodMuonsPt && fGoodMuonsEta && (fb.Muon_mvaLowPt[mmindex1] > fCuts[0]->muonbdt) && (fb.Muon_mvaLowPt[mmindex1] > fCuts[0]->muonbdt));
  fGoodRealAmsID = ((fb.Muon_mvaLowPt[mmindex1] > -2.5) && (fb.Muon_mvaLowPt[mmindex1] > -2.5) && fGoodMuonsPt && fGoodMuonsEta && (fb.Muon_mvaLowPt[mmindex1] < fCuts[0]->muonbdt) && (fb.Muon_mvaLowPt[mmindex1] < fCuts[0]->muonbdt));
  fIsCowboy = true;

  //&& fGoodPt && fGoodEta && fGoodAlpha && fGoodChi2 && fGoodFLS && fGoodCloseTrack && fGoodDocaTrk && fGoodIso && fGoodM1Iso  && fGoodM2Iso &&                     
  //if(fIsMC)cout<<"MC Atleast good bdt HLT entering "<<fMode<<" GoodPt "<<fGoodTracksPt<<" goodeta "<<fGoodTracksEta<<" goodmuonpt "<<fGoodMuonsPt<<" goodcmssw "<<fGoodCmssw<<" goodcand "<<fGoodCandAna<<" goodpresel "<<fPreselection<<" goodhlt1 "<<fGoodHLT<<endl;
  //else cout<<"Data Atleast good bdt HLT entering "<<fMode<<" GoodPt "<<fGoodTracksPt<<" goodeta "<<fGoodTracksEta<<" goodmuonpt "<<fGoodMuonsPt<<" goodcmssw "<<fGoodCmssw<<" goodcand "<<fGoodCandAna<<" goodpresel "<<fPreselection<<" goodhlt1 "<<fGoodHLT<<endl;
  ...
  double mass = 0.;
  if(BU2JPSIKP == fMode) mass = fb.m[mom_l];
  else mass = fb.mm_mass[mom_l];
  double ps   = 1.;
  //cout<<"entering to the loopfunction 1"<<endl;
  if (!(fGoodDcand) ) return; 
  for (unsigned int im = 0; im < fHistStrings.size(); ++im) {
    fhMassAbsNoCuts[fHistStrings[im]][fChan]->Fill(mass);
    if(BU2JPSIKP == fMode){
      fhAccAll[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.eta[mom_l]), fb.pt[mom_l]);
      fhAccPtAll[fHistStrings[im]][fChan]->Fill(fb.pt[mom_l]);
      fhAccEtaAll[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.eta[mom_l]));
    } else{
      fhAccAll[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.mm_kin_eta[mom_l]), fb.mm_kin_pt[mom_l]);
      fhAccPtAll[fHistStrings[im]][fChan]->Fill(fb.mm_kin_pt[mom_l]);
      fhAccEtaAll[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.mm_kin_eta[mom_l]));
    }
  }
  ...
  for (unsigned int im = 0; im < fHistStrings.size(); ++im) {
    if(BU2JPSIKP == fMode){
      fhAccPass[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.eta[mom_l]), fb.pt[mom_l]);
      fhAccPtPass[fHistStrings[im]][fChan]->Fill(fb.pt[mom_l]);
      fhAccEtaPass[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.eta[mom_l]));
    } else {
      fhAccPass[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.mm_kin_eta[mom_l]), fb.mm_kin_pt[mom_l]);
      fhAccPtPass[fHistStrings[im]][fChan]->Fill(fb.mm_kin_pt[mom_l]);
      fhAccEtaPass[fHistStrings[im]][fChan]->Fill(TMath::Abs(fb.mm_kin_eta[mom_l]));
    }
  ...
  bool  Print = false;
  if(Print){
  if(fIsMC)cout<<"MC Run "<<fb.run<<" event "<<fb.evt<<" nbkmm "<<fb.nbkmm<<" mass "<<fb.m[mom_l]<<" m1pt "<<fb.m1pt[mom_l]<<" m2pt "<<fb.m2pt[mom_l]<<" m1eta "<<fb.m1eta[mom_l]<<" m2eta "<<fb.m2eta[mom_l]<<" PDG "<<fb.bkmm_pdgId[mom_l]<<" hlt1 "<<fb.hlt1<<" hlt2 "<<fb.hlt2<<" hlt3 "<<fb.hlt3<< " kaon PDG ID "<<fb.bkmm_k1_pdgId[mom_l]<<"  K1eta "<<fb.k1eta[mom_l]<<" k1pt "<<fb.k1pt[mom_l]<<" Chan "<<fChan<<endl;
  else {cout<<"Data Run "<<fb.run<<" event "<<fb.evt<<" nbkmm "<<fb.nbkmm<<" mass "<<fb.m[mom_l]<<" m1pt "<<fb.m1pt[mom_l]<<" m2pt "
	    <<fb.m2pt[mom_l]<<" m1eta "<<fb.m1eta[mom_l]<<" m2eta "<<fb.m2eta[mom_l]<<" K1eta "<<fb.k1eta[mom_l]
	    <<" hlt "<<fb.hlt1<< " kaon "<<fb.k1pt[mom_l]<<" Bpt "<<fb.pt[mom_l]<<" k1pt "<<fb.k1pt[mom_l]
  ...
  // 	  fhNormC[fHistStrings[0]][fChan]->Fill(mass, -0.1, ps);//changed by me
  // 	  fhNormC[fHistStrings[0]][fChan]->Fill(mass, 0.1);//
  // 	  fhNormC[fHistStrings[0]][fChan]->Fill(mass, ps+0.1);//

  // 	  fhW8Norm[fHistStrings[0]][fChan]->Fill(mass, -0.1, ps);
  // 	  fhW8Norm[fHistStrings[0]][fChan]->Fill(mass, 0.1, 1);
  // 	  fhW8Norm[fHistStrings[0]][fChan]->Fill(mass, ps+0.1, 1);
  // 	  fhW8NormC[fHistStrings[0]][fChan]->Fill(mass, -0.1, ps);
  // 	  fhW8NormC[fHistStrings[0]][fChan]->Fill(mass, 0.1, 1);
  // 	  fhW8NormC[fHistStrings[0]][fChan]->Fill(mass, ps+0.1,1);
  ...
  if (fGoodQ       
	...
	if (fGoodMuonsID){
	  ...
	  if (fGoodHLT) {
	    ...
	    //std::cout<<" Run "<<fb.run<<" event "<<fb.evt<<" nbkmm "<<nloop<<" mass "<<mass<<"MuonId "<<fb.Muon_mvaLowPt[mmindex1]<<" 2: "<<fb.Muon_mvaLowPt[mmindex2]<<" BDT :"<<fBDT<<" cut:"<<(i-2)*0.01<<std::endl;
	    ...
	    if ((fMode == BS2JPSIPHI)       
		|| (fMode == BU2JPSIKP)) {
	      //cout<<"Run "<<fb.run<<" event "<<fb.evt<<" good bdt entering "<<fMode<<" mass "<<mass<< " ps "<<ps<<" goodBDt "<<goodBDT<<" histstring "<<fHistStrings[i]<<endl;
