...
void plotResultsnew::calculateRareBgNumbers(int chan) {
  ...
  for (map<string, dataset*>::iterator it = fDS.begin(); it != fDS.end(); ++it) {
    ...
    TH1D *hu = (TH1D*)fHistFile->Get(namemuon.c_str());
    TH1D *hw = (TH1D*)fHistFile->Get(namemuon.c_str());
    //TH1D *hu = (TH1D*)fHistFile->Get(u8name.c_str());
    //TH1D *hw = (TH1D*)fHistFile->Get(w8name.c_str());
    TH1D *ha = (TH1D*)fHistFile->Get(amname.c_str());
    
    cout<<"u8name "<<u8name.c_str()<< " entry "<<hu->GetEntries()<<endl;
    ...
    if (0 == nmuons) {
      eTot     = eAccAna * (massIntegral(hu, ALL, chan) / utot) * triggerEffAve;
    } else {
    ...
    if (1){
      cout << "acc = " << eAccAna << " integral = " << massIntegral(ha, ALL, chan) << " utot = " << utot << " eff = " << triggerEffAmsAve << endl;
      cout << "acc = " << eAccAna << " integral u8 = " << massIntegral(hu, ALL, chan) << " utot = " << utot << " muid eff = " << triggerEffAve << endl;
      cout << "signal eff= " << triggerEffSignal <<" etot "<< eTot <<" genyield " << genYield << " trigger signal "<< fBsmmNumbers[chan].fEffTrigMC.val << endl;
      cout << "Efficiency by hand " << (double)htrg->GetEntries()/hm->GetEntries() <<" \t "<<utot_s/utot<< endl;
      cout << "acc ="<< eAccAna << " integral u8 = " << massIntegral(hu, ALL, chan) << " utot = " << utot << " Acceptance "<< Acceptance<< " ac*tot "<<Acceptance*eAccAna<<endl;
    }
...
void plotResultsnew::loopFunction1() {
  ...
  if(BU2JPSIKP == fMode){
    ...
    fGoodBDT        = (fb.mm_bdt > pCuts->bdtCut);
    fGoodCmssw      = (fb.Muon1_pt > 4.) && (fb.Muon2_pt > 4.)   && (fb.mm_kin_sl3d > 4.) && (fb.mm_kin_vtx_chi2dof < 5) && fb.mm_kin_mass>4.9 && fb.mm_kin_mass<5.9;
    fGoodHLT        = fb.hlt3 ;
 ...
 fGoodMuonsID = ((fb.Muon1_softId==1) && (fb.Muon2_softId==1));
