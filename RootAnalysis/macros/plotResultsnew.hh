...
#ifndef PLOTRESULTSNEW_h
#define PLOTRESULTSNEW_h
...
// ----------------------------------------------------------------------
class plotResultsnew: public plotClassnew {

public :
  plotResultsnew(std::string dir = "results",
	      std::string files = "plotResults.2016BF.files",
	      std::string cuts = "baseCuts.2016.cuts",
	      std::string setup = "",
	      int year = 0);
  virtual ~plotResultsnew();
  ...
  //void maketoyprediction(double v1, double v2, double mtoy, double Rtoy);
  ...
  void   loopOvertree_trig(TTree *t,  int nevts = -1, int nstart = 0);
  void   loopOverChain(TChain *t, int ifunc, int nevts = -1, int nstart = 0);
...
private:
  ...
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_womuid_1;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_womuid_2;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_womuid_3;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_womuid_4;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_womuid_5;

  std::map<std::string, std::vector<TH1D*> > fhMassWithloosemuidforsinglemuonCuts, fhMassWithmediummuidforsinglemuonCuts, fhMassWithsoftmvamuidforsinglemuonCuts;
  std::map<std::string, std::vector<TH1D*> > fhMassWithloosemuidfordoublemuonCuts, fhMassWithmediummuidfordoublemuonCuts, fhMassWithsoftmvamuidfordoublemuonCuts;
  std::map<std::string, std::vector<TH1D*> > fhMassWithMuonCuts;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_1;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_2;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_3;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_4;
  std::map<std::string, std::vector<TH1D*> > fhMassWithTriggerCuts_5;
  ...
  std::map<std::string, std::vector<TH1D*> > fhNorm_1D, fhNorm_1D_Wps;
  std::map<std::string, std::vector<TH1D*> > fhNorm_1D_triglxy_scale;
