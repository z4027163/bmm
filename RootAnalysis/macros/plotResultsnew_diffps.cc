...
void plotResultsnew::loopFunction1() {
  ...
  fGoodTracksPt   = true;
  fGoodTracksEta  = true;
  fGoodQ          = (fb.Muon1_Charge*fb.Muon2_Charge < 0);
  fIsCowboy = true;
 
  if(!(fIsMC)){if((fb.m<4.9 || fb.m>6.0))return;}
  if (fIsMC) {
    if (fb.Muon1_pt < fAccPt) fGoodAcceptance = false;
    if (fb.Muon2_pt < fAccPt) fGoodAcceptance = false;
    if (TMath::Abs(fb.Muon1_eta) > fAccEtaGen) fGoodAcceptance = false;
    if (TMath::Abs(fb.Muon2_eta) > fAccEtaGen) fGoodAcceptance = false;
  ...
  double m1 = TMath::Abs(fb.Muon1_eta);
  double m2 = TMath::Abs(fb.Muon2_eta);
  if (m2 > m1) m1 = m2;
  ...
  if(BU2JPSIKP == fMode){
    ...
    //fGoodCmssw   =  (fb.m1pt > 4.) && (fb.m2pt > 4.)   && (fb.fls3d > 13.) && (fb.docatrk >0.015) && (fb.chi2dof < 2.2) && (fb.pt>6.5) && (fb.mm_doca <0.1);
    fGoodCmssw   =  (fb.Muon1_pt > 4.) && (fb.Muon2_pt > 4.)   && (fb.fls3d > 4.) && (fb.chi2dof < 5);
    //    fGoodCandAna =  (fb.alpha < 0.05) && (fb.mm_closetrk<2.0) && (TMath::Abs(fb.pvip/fb.pvipErr) < 2.0) && (TMath::Abs(fb.pvip) < 0.008) && (TMath::Abs(fb.pvlip/fb.pvlipErr) < 99.0) && (TMath::Abs(fb.pvlip) < 99.) &&(fb.mm_iso > 0.80) && (fb.mm_m1iso > 0.80) && (fb.mm_m2iso > 0.80);
    fGoodHLT        = fb.hlt1 ;
  } else if(BS2JPSIPHI == fMode) {
    
    fGoodBDT        = (fb.mm_bdt > pCuts->bdtCut);
    fGoodCmssw   =  (fb.Muon1_pt > 4.) && (fb.Muon2_pt > 4.)   && (fb.fls3d > 4.) && (fb.chi2dof < 5) && (fb.bkkmm_kk_mass-1.02)<0.01;
    //fGoodCandAna =  (fb.alpha < 0.05) && (fb.mm_closetrk<2.0) && (TMath::Abs(fb.pvip/fb.pvipErr) < 2.0) && (TMath::Abs(fb.pvip) < 0.008) && (TMath::Abs(fb.pvlip/fb.pvlipErr) < 99.0) && (TMath::Abs(fb.pvlip) < 99.) &&(fb.mm_iso > 0.80) && (fb.mm_m1iso > 0.80) && (fb.mm_m2iso > 0.80);
    ...
    fGoodCmssw   =  (fb.Muon1_pt > 4.) && (fb.Muon2_pt > 4.)   && (fb.mm_kin_sl3d > 4.) && (fb.mm_kin_vtx_chi2dof < 5);
    //    fGoodCandAna =  (fb.mm_kin_alpha < 0.05) && (fb.mm_closetrk<2.0) && (TMath::Abs(fb.mm_kin_pvip/fb.mm_kin_pvipErr) < 2.0) && (TMath::Abs(fb.mm_kin_pvip) < 0.008) && (TMath::Abs(fb.mm_kin_pvlip/fb.mm_kin_pvlipErr) < 99.0) && (TMath::Abs(fb.mm_kin_pvlip) < 99.) &&(fb.mm_iso > 0.80) && (fb.mm_m1iso > 0.80) && (fb.mm_m2iso > 0.80);
 ...
 fGoodMuonsID = ( fGoodMuonsPt && fGoodMuonsEta && (fb.Muon1_softId==1) && (fb.Muon2_softId==1));

  //fGoodMuonsID = ( fb.Muon_isGlobal && fb.Muon_isGlobal[mmindex2] && fGoodMuonsPt && fGoodMuonsEta && (Muon_softId==1) && (Muon_softId[mmindex2]==1));// && (fb.Muon_mvaLowPt > fCuts[0]->muonbdt) && (fb.Muon_mvaLowPt[mmindex2] > fCuts[0]->muonbdt));
